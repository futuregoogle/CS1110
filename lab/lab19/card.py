...
class Card:
    ...
    def __init__(self, s=0, r=1, alt=None):
        ...
        # to allow for sub-classing of the class Card where there can be more
        # suits or ranks than in the usual Card class. (Foreshadowing a later
        # lab.)
        if alt: # We overwrite the given `s` and `r`
            s = 'CDHS'.index(alt[1])
            r = ' A23456789TJQK'.index(alt[0])
        assert isinstance(s, int), "Card(): Did not get int for input s="+repr(s)
        assert isinstance(r, int), "Card(): Did not get int for input r="+repr(r)
        self.suit = s
        self.rank = r
    ...
    def __gt__(self, other):
        ...
        False if `other` is a Card and neither condition above holds;
